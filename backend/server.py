...
def run_pipeline(job_id: str, initial_state: dict):
    ...
    try:
        ...
        # ── Persist data-source state for query loop ──────────────────────────
        # Store df, engine, mongo_client etc so /api/query can reuse them
...
async def natural_language_query(job_id: str, body: QueryRequest):
    """
    Intelligent query engine — runs against the actual data source (CSV / SQL / MongoDB).
    AI decides whether a chart is needed; if so, returns it with chart data for the lightbox.
    """
    ...
    # Build a rich context string from the pipeline outputs
    source_context = "\n\n".join(filter(None, [
